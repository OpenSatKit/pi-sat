...
void FILE_Constructor(FILE_Class*  FilePtr, INITBL_Class* IniTbl)
{
   ...
   CFE_MSG_Init(&File->InfoPkt.TlmHeader.Msg, (CFE_SB_MsgId_t)INITBL_GetIntConfig(File->IniTbl, CFG_FILE_INFO_TLM_MID), 
                sizeof(FILE_InfoPkt));
...
bool FILE_ConcatenateCmd(void* DataObjPtr, const CFE_SB_Buffer_t* SbBufPtr)
{
   ...
   const FILE_ConcatenateCmdMsg* ConcatenateCmd = (FILE_ConcatenateCmdMsg *) SbBufPtr;
...
bool FILE_CopyCmd(void* DataObjPtr, const CFE_SB_Buffer_t* SbBufPtr)
{
   ...
   const FILE_CopyCmdMsg*  CopyCmd = (FILE_CopyCmdMsg *) SbBufPtr;
   FileUtil_FileInfo FileInfo;
   int32  SysStatus;   
...
bool FILE_DecompressCmd(void* DataObjPtr, const CFE_SB_Buffer_t* SbBufPtr)
{
   ...
   FILE_DecompressCmdMsg* DecompressCmd = (FILE_DecompressCmdMsg *) SbBufPtr;  /* CFE_PSP_Decompress() doesn't use const */
   FileUtil_FileInfo FileInfo;
   int32  CfeStatus;
   bool   RetStatus = false;
...
bool FILE_DeleteCmd(void* DataObjPtr, const CFE_SB_Buffer_t* SbBufPtr)
{
   ...
   const FILE_DeleteCmdMsg* DeleteCmd = (FILE_DeleteCmdMsg *) SbBufPtr;
   FileUtil_FileInfo FileInfo;
   int32  SysStatus;
   bool   RetStatus = false;
...
bool FILE_MoveCmd(void* DataObjPtr, const CFE_SB_Buffer_t* SbBufPtr)
{
   ...
   const FILE_MoveCmdMsg* MoveCmd = (FILE_MoveCmdMsg *) SbBufPtr;
   FileUtil_FileInfo FileInfo;
   int32  SysStatus;   
...
bool FILE_RenameCmd(void* DataObjPtr, const CFE_SB_Buffer_t* SbBufPtr)
{
   ...
   const FILE_RenameCmdMsg* RenameCmd = (FILE_RenameCmdMsg *) SbBufPtr;
   FileUtil_FileInfo FileInfo;
   int32  SysStatus;   
   bool   RetStatus = false;
...
**
*/
bool FILE_SendInfoPktCmd(void* DataObjPtr, const CFE_SB_Buffer_t* SbBufPtr)
{
   
   FILE_SendInfoPktCmdMsg* SendInfoPktCmd = (FILE_SendInfoPktCmdMsg *) SbBufPtr;
   FileUtil_FileInfo FileInfo;
   uint32  Crc;
   bool    RetStatus = false;
   ...
   if (FILEUTIL_FILE_EXISTS(FileInfo.State)) {
      ...
      if (RetStatus == true) {
         ...
         CFE_SB_TimeStampMsg(&(File->InfoPkt.TlmHeader.Msg));
         CFE_SB_TransmitMsg(&(File->InfoPkt.TlmHeader.Msg), true);            
...
bool FILE_SetPermissionsCmd(void* DataObjPtr, const CFE_SB_Buffer_t* SbBufPtr)
{
   ...
   const FILE_SetPermissionsCmdMsg* SetPermissionsCmd = (FILE_SetPermissionsCmdMsg *) SbBufPtr;
   FileUtil_FileInfo FileInfo;
   int32  SysStatus;   
   bool   RetStatus = false;
   ...
   if (SysStatus == OS_SUCCESS) {
      ...
      if (SysStatus == OS_SUCCESS) {
         ...
         if (SysStatus == OS_SUCCESS) {
            ...
            sprintf(EventErrStr,"Concatenate file cmd error: Error opening target file %s. Open status %d", SrcFile2, SysStatus);
         ...
         sprintf(EventErrStr,"Concatenate file cmd error: Error opening source file %s. Open status %d", SrcFile2, SysStatus);
