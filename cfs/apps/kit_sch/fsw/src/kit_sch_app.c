...
static void ProcessCmdPkt(CFE_SB_Buffer_t* SbBufPtr);
...
void KIT_SCH_AppMain(void)
{
   ...
   */
   ...
   while (CFE_ES_RunLoop(&RunStatus) == true) {
      ...
      Status = CFE_SB_ReceiveBuffer(&SbBufPtr, KitSch.CmdPipe, CFE_SB_POLL);
        
      if (Status == CFE_SUCCESS) {
          
         ProcessCmdPkt(SbBufPtr);
      }
   ...
   CFE_EVS_SendEvent(KIT_SCH_APP_EXIT_EID, CFE_EVS_EventType_CRITICAL, "KIT_SCH App: terminating, err = 0x%08X", Status);
...
static int32 InitApp(void)
{
    ...
    */
    ...
    CFE_SB_Subscribe(CFE_SB_ValueToMsgId(KIT_SCH_CMD_MID), KitSch.CmdPipe);
    CFE_SB_Subscribe(CFE_SB_ValueToMsgId(KIT_SCH_SEND_HK_MID), KitSch.CmdPipe);
    ...
    CFE_MSG_Init(&KitSch.HkPkt.TlmHeader.Msg, KIT_SCH_HK_TLM_MID, KIT_SCH_HK_TLM_LEN);
   ...
   switch (CFE_SB_MsgIdToValue(MsgId)) {
      ...
      default:
         CFE_EVS_SendEvent(KIT_SCH_APP_MID_ERR_EID, CFE_EVS_EventType_ERROR,
                           "Received invalid command packet,MID = 0x%x",CFE_SB_MsgIdToValue(MsgId));
